import React from 'react';
import { Service } from '../types';
import { ArrowRight } from 'lucide-react';
import { motion } from 'framer-motion';

interface ServicesProps {
  onRequestDemo: (service: Service) => void;
  onServiceClick?: (service: Service) => void;
}

const colors = [
  '#FF6B6B', // red
  '#4ECDC4', // teal
  '#FFD93D', // yellow
  '#6A0572', // purple
  '#FFD700', // gold
  '#558B6E', // green
  '#FF8C00', // dark orange
  '#00BFFF', // deep sky blue
  '#D72631', // strong red
  '#8A2BE2', // blue violet
  '#20B2AA', // light sea green
];

const servicesData: Service[] = [
  {
    label: 'Digital Marketing',
    link: '',
    description: 'Expert strategies for digital growth.',
    features: ['SEO', 'Content Marketing', 'Social Media'],
    image: '/images/dm.png',
  },
  {
    label: 'UI and UX Design',
    link: '',
    description: 'Creating user-friendly and beautiful designs.',
    features: ['Wireframes', 'Prototyping', 'User Testing'],
    image: '/images/uiux.png',
  },
  {
    label: 'Graphic Design',
    link: '',
    description: 'Crafting visual content that communicates.',
    features: ['Branding', 'Print Design', 'Illustrations'],
    image: '/images/gd.png',
  },
  {
    label: 'Web Solutions',
    link: '',
    description: 'Robust and scalable web applications.',
    features: ['E-commerce', 'CMS', 'Custom Development'],
    image: '/images/wd.jpg',
  },
  {
    label: 'AI and ML',
    link: '',
    description: 'Intelligent systems for modern problems.',
    features: ['Predictive Analytics', 'Chatbots', 'Automation'],
    image: '/images/aiml.jpg',
  },
  {
    label: 'Cybersecurity',
    link: '',
    description: 'Protecting your digital assets.',
    features: ['Vulnerability Assessment', 'Penetration Testing', 'Incident Response'],
    image: '/images/cysec.jpg',
  },
  {
    label: 'Cloud Computing',
    link: '',
    description: 'Scalable cloud infrastructure services.',
    features: ['AWS', 'Azure', 'Google Cloud'],
    image: '/images/cc.jpg',
  },
  {
    label: 'Student Project',
    link: '',
    description: 'Support and development for academic projects.',
    features: ['Guidance', 'Implementation', 'Presentation'],
    image: '/images/sp.jpg',
  },
  {
    label: 'Automation',
    link: '',
    description: 'Enhancing efficiency through automation.',
    features: ['Process Automation', 'Scripting', 'Tools Integration'],
    image: '/images/at.png',
  },
  {
    label: 'PCB Building',
    link: '',
    description: 'Design and manufacturing of PCBs.',
    features: ['PCB Design', 'Prototyping', 'Testing'],
    image: '/images/pcb.jpg',
  },
  {
    label: 'Blockchain',
    link: '',
    description: 'Secure and decentralized solutions.',
    features: ['Smart Contracts', 'DApps', 'Cryptocurrency'],
    image: '/images/bc.jpg',
  },
];

const headlineVariants = {
  hidden: { opacity: 0, y: -20 },
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.8, ease: 'easeOut' },
  },
};

const containerVariants = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.12,
      delayChildren: 0.3,
    },
  },
};

const itemVariants = {
  hidden: { opacity: 0, y: 20, scale: 0.95 },
  visible: (color: string) => ({
    opacity: 1,
    y: 0,
    scale: 1,
    transition: { type: 'spring', stiffness: 80, damping: 12, duration: 0.6 },
    boxShadow: `0 0 15px 3px ${color}`,
  }),
};

const Services: React.FC<ServicesProps> = ({ onRequestDemo, onServiceClick }) => {
  const handleRequestDemo = (service: Service) => {
    onRequestDemo(service);
  };

  const handleServiceClick = (service: Service) => {
    if (onServiceClick) {
      onServiceClick(service);
    }
  };

  return (
    <section className="py-16 bg-card-light dark:bg-card-dark rounded-xl shadow-soft max-w-8xl mx-auto px-6">
      {/* Top headline section - full width, centered */}
      <motion.div className="mb-12 text-center" initial="hidden" animate="visible" variants={headlineVariants}>
        <h2 className="text-4xl md:text-5xl font-extrabold mb-6 text-primary dark:text-white">
          Utilize
          <span className="block">the Technology with</span>
          <span className="block">SkyZuri Techbridge</span>
        </h2>
        <p className="text-lg text-gray-700 dark:text-gray-300 max-w-2xl mx-auto">Explore our wide range of programs and embark on a journey to craft your future with excellence.</p>
      </motion.div>

      {/* Bottom grid of service cards - full width */}
      <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8" variants={containerVariants} initial="hidden" animate="visible">
          {servicesData.map((service, index) => (
            <motion.div
              key={service.label}
              onClick={() => handleServiceClick(service)}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="relative w-full rounded-3xl overflow-hidden bg-black/5 dark:bg-white/5 shadow-lg focus:outline-none cursor-pointer"
            >
              {/* Image area */}
              <div className="w-full h-56 md:h-64 bg-gray-100 dark:bg-gray-800">
                {service.image ? (
                  <img src={service.image} alt={service.label} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-gray-400">No image</div>
                )}
              </div>

              {/* Content area */}
              <div className="p-6">
                <h3 className="text-xl font-semibold mb-2 text-gray-800 dark:text-white">{service.label}</h3>
                <p className="text-gray-600 dark:text-gray-300 mb-4 text-sm leading-relaxed">{service.description}</p>
                <div className="flex justify-end mt-4">
                  <ArrowRight size={20} className="text-gray-400 dark:text-gray-500" />
                </div>
              </div>
            </motion.div>
          ))}
      </motion.div>
    </section>
  );
};

export default Services;
